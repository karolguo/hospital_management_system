...
<div class="col-md-12 col-sm-12">
    <div class="blog-content">
        ...
        <%--病例开始--%>
<c:forEach items="${Case}" var="c">
<div class="single-Blog" style="margin-top: 40px">
        ...
        <ul class="blog-comments-box">
            <ul class="blog-comments-box">
                <li>
                    <h3>
                        <c:choose>
                            <c:when test="${fn:substring(c.medDate,5,7)==1}">
                                Jan
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==2}">
                                Feb
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==3}">
                                Mar
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==4}">
                                Apr
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==5}">
                                May
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==6}">
                                June
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==7}">
                                July
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==8}">
                                Aug
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==9}">
                                Sept
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==10}">
                                Oct
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==11}">
                                Nov
                            </c:when>
                            <c:when test="${fn:substring(c.medDate,5,7)==12}">
                                Dec
                            </c:when>
                        </c:choose></h3>
                    <h2>${fn:substring(c.medDate,5,7)}</h2>${fn:substring(c.medDate,'',4)}</li>
            </ul>
        </ul>
    </div>
    ...
    <div class="single-blog-right">
        <div class="blog-author">
            <ul>
                <li>By <a href="#">${c.doctorName}</a></li>
                <li>In <a href="#">${c.deptName}</a></li>
            </ul>
        </div>
        <div class="blog-content">
            <p>${c.userName}</p>
            <h2>症状:${c.medOrder}</h2>
            <p>检查花费：${c.mrCheckPrice}</p>
            <!-- Read more btn -->
            <a class="wpcf7-submit button--itzel" href="card/findDetailedCase?mrId=${c.mrId}">
